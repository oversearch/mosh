...
{
  fprintf( stream,
           "Usage: %s new [-A] [-s] [-v] [-i LOCALADDR] [-p PORT[:PORT2]] [-c COLORS] [-l NAME=VALUE] [-- COMMAND...]\n",
           argv0 );
}
...
int main( int argc, char* argv[] )
{
  ...
  try {
    return run_server(
      desired_ip, desired_port, command_path, command_argv, colors, verbose, with_motd, with_agent_fwd );
                       ...
                       bool with_agent_fwd )
{
  ...
  if ( child == 0 ) {
    ...
    try {
      serve( master, pipes[1], terminal, *network, network_timeout, network_signaled_timeout, agent );
    } catch ( const Network::NetworkException& e ) {
      fprintf( stderr, "Network exception: %s\n", e.what() );
    } catch ( const Crypto::CryptoException& e ) {
      fprintf( stderr, "Crypto exception: %s\n", e.what() );
